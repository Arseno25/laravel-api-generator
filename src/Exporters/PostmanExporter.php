...
final class PostmanExporter
{
    ...
    public function export(array $schema, string $baseUrl): array
    {
        $collection = [
            'info' => [
                ...
                'description' => 'Auto-generated by Laravel API Magic v'.($schema['version'] ?? '1.0.0'),
        ...
        foreach ($schema['endpoints'] ?? [] as $path => $methods) {
            foreach ($methods as $method => $endpoint) {
                ...
                $folders[$folder][] = $this->buildRequest($method, $path, $endpoint, $baseUrl);
            }
        }
     ...
     * @return array<string, mixed>
     */
    private function buildRequest(string $method, string $path, array $endpoint, string $baseUrl): array
    {
        $summary = $endpoint['summary'] ?? Str::title(str_replace(['/', '{', '}', '-', '_'], [' ', '', '', ' ', ' '], $path));
