...
it('normalizes schema-qualified table names', function () {
    ...
    expect($this->inferrer->normalizeTableName('main.products'))->toBe('products');
    expect($this->inferrer->normalizeTableName('[dbo].[orders]'))->toBe('orders');
});
...
it('normalizes schema-qualified table listings in parser', function () {
    Schema::shouldReceive('getTableListing')
        ->once()
        ->andReturn(['main.migrations', 'main.users', 'public.products', '[dbo].[orders]']);

    $parser = new DatabaseSchemaParser(new DatabaseColumnMetadataInferrer);

    expect($parser->getTables())->toBe(['users', 'products', 'orders']);
});
